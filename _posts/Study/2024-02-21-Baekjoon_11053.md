...
> - 만약 `a[i]`가 `a[j]`보다 크다면, `num[i] = num[j]+1` 
> - 이를 `a[i]` 이전의 모든 원소들을 순회하면서 `num[i]` 값을 업데이트
> - 즉, `a[i]`보다 작은 원소들 중 가장 큰 원소를 `k`라 했을 때 `num[i] = num[k]+1`이 될 것이다.

여기에 **Dynamic Programming**의 원리가 사용되는데, 이는 간단하게 말하면 **"기억하며 풀기"**이다. <br>
이것 또한, 나무위키에 잘 설명되어있다. [Dynamic Programming 참고](https://namu.wiki/w/%EB%8F%99%EC%A0%81%20%EA%B3%84%ED%9A%8D%EB%B2%95)
