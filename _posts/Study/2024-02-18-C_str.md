...
- `strchr()` 함수는 문자열에서 검색한 문자가 **처음으로 등장하는 곳의 포인터**를 반환
- `strrchr()` 함수는 문자열에서 검색한 문자가 **마지막으로 등장하는 곳의 포인터**를 반환
...
### ctype.h
> 문자(character)와 관련된 함수들을 선언하는 헤더 파일 _(Character Type Header)_
