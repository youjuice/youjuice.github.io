...
DFS와 BFS를 이해하고 있다면 쉽게 풀릴 듯 하다.. 생각보다는 어렵지 않았음
...
- 우선 바이러스를 퍼뜨리기 위해 **바이러스 좌표들**을 `queue`에 삽입한다.
...
def make_wall_combinations():
    ...
    for walls in wall_combinations:
        ...
        bfs(tmp_lab)
```
- `combinations` 함수를 활용해 `empty`에서 3개의 좌표를 선택한 조합들을 만들어준다.
- 각 조합에 대해 `tmp_lab`을 복사한 후, 해당 좌표에 벽을 세운 다음 `bfs(tmp_lab)` 함수를 호출한다.
...
- `make_wall_combinations()` 함수를 호출하여 모든 벽을 세우는 경우의 수를 탐색하고 **최대 안전 영역의 크기**를 받아온다.
...
def make_wall_recursion(count):
    ...
    for i in range(x):
        for j in range(y):                    # 전체 그래프를 탐색하면서
            if lab[i][j] == 0:                # 빈칸이라면
                lab[i][j] = 1                 # 벽 세우기
                make_wall_recursion(count+1)  # 벽 count + 1
                lab[i][j] = 0                 # 벽 허물기
...
> - 이전 단계로 돌아가면서, **마지막으로 세운 벽**을 **허물고** 다음 빈 칸 탐색
